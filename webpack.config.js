...
const serverWeb = {
  ...
  resolve: {
    ...
    extensions: [".ts", ".js"], // support ts-files and js-files
    alias: {
      "./services/yamlFormatter": path.resolve(
        __dirname,
        "./build/polyfills/yamlFormatter.js"
      ), // not supported for now. prettier can run in the web, but it's a bit more work.
      "vscode-json-languageservice/lib/umd":
        "vscode-json-languageservice/lib/esm",
    },
    fallback: {
      path: require.resolve("path-browserify/"),
  ...
  plugins: [
    new webpack.ProvidePlugin({
      process: path.resolve(
        path.join(__dirname, "node_modules/process/browser.js")
