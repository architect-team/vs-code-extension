...
export function customTagsTest(): void {
  describe("Verify extension's custom tags", () => {
    it("YAML custom tags works as expected", async function () {
      ...
      await setting
        .findElement(By.className("edit-in-settings-button"))
        .click();
      ...
      await driver
        .actions()
        .sendKeys(Key.chord(TextEditor.ctlKey, "s"))
        .perform();
      ...
      const contentAssist: ContentAssist | void = await new TextEditor().toggleContentAssist(
        true
      );
      ...
      if (contentAssist instanceof ContentAssist) {
        ...
        if (!hasItem) {
          throw new Error(
            "The 'customTag1' custom tag did not appear in the content assist's suggestion list."
          );
        }
      } else {
        throw new Error(
