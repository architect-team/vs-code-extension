...
describe("Status bar should work in multiple different scenarios", () => {
  ...
  beforeEach(() => {
    ...
    createStatusBarItemStub = sandbox.stub(
      vscode.window,
      "createStatusBarItem"
    );
    onDidChangeActiveTextEditorStub = sandbox.stub(
      vscode.window,
  ...
  it("Should create status bar item for JSON Schema", async () => {
    ...
    createJSONSchemaStatusBarItem(
      context,
      (clcStub as unknown) as CommonLanguageClient
    );

    expect(registerCommandStub).calledOnceWith("yaml.select.json.schema");
    expect(createStatusBarItemStub).calledOnceWith(
  ...
  it("Should update status bar on editor change", async () => {
    ...
    clcStub.sendRequest.resolves([
      { uri: "https://foo.com/bar.json", name: "bar schema" },
    ]);
  ...
  it("Should inform if there are more than one schema", async () => {
    ...
    expect(statusBar.tooltip).to.equal(
      "Multiple JSON Schema used to validate this file, click to select one"
    );
    expect(statusBar.backgroundColor).to.eql({
  ...
  it("Should show JSON Schema Store schema version", async () => {
    ...
    createStatusBarItemStub.returns(statusBar);
    onDidChangeActiveTextEditorStub.returns({
      document: { uri: vscode.Uri.parse("/foo.yaml") },
    });
    clcStub.sendRequest
      .withArgs(
        sinon.match.has("method", "yaml/get/jsonSchema"),
        sinon.match("/foo.yaml")
      )
      .resolves([{ uri: "https://foo.com/bar.json", name: "bar schema" }]);
    clcStub.sendRequest
      .withArgs(
        sinon.match.has("method", "yaml/get/all/jsonSchemas"),
        sinon.match.any
      )
  ...
  it("Should show JSON Schema Store schema version, dont include version", async () => {
    ...
    clcStub.sendRequest
      ...
      .resolves([
        { uri: "https://foo.com/bar.json", name: "bar schema(1.0.0)" },
      ]);
    clcStub.sendRequest
      .withArgs(
        sinon.match.has("method", "yaml/get/all/jsonSchemas"),
